...
public class RegistrySubCommand implements SubCommand {
    ...
    public ArgumentBuilder<CommandSourceStack, ?> build(
        ...
        return builder.then(
                ...
                .then(
                        Commands.literal("scent")
                                .executes(ctx -> executeList(ctx, RegistryType.SCENT)))
                .then(Commands.literal("nose").executes(ctx -> executeList(ctx, RegistryType.NOSE)))
    ...
    private int showUsage(CommandContext<CommandSourceStack> context) {
        ...
        source.sendSuccess(
                () -> Texts.lit("§e  /aromatest registry structure §8- List structures"), false);
