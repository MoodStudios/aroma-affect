...
public class GiveVariantSubCommand implements SubCommand {
    ...
    private static final SuggestionProvider<CommandSourceStack> VARIANT_SUGGESTIONS =
            (ctx, builder) -> {
                for (Identifier id : NoseVariantRegistry.all().keySet()) {
    ...
    private int execute(CommandContext<CommandSourceStack> context) {
        ...
        Item customNose = CustomNoseRegistry.getCustomNose();
        if (customNose == null) {
            source.sendFailure(Texts.lit("§cCustomNoseItem not registered"));
            return 0;
        }
        ItemStack stack = CustomNoseItem.stackFor(customNose, id, variantOpt.get());
