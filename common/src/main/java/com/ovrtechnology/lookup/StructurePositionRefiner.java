...
/**
 * Refines the approximate position returned by structure lookup to point at
 * an actual solid block belonging to the structure rather than a generic chunk position.
 */
public final class StructurePositionRefiner {
    ...
    /**
     * Natural terrain blocks to exclude when searching for structure blocks.
     */
    private static final Set<Block> NATURAL_BLOCKS = Set.of(
            Blocks.AIR, Blocks.CAVE_AIR, Blocks.VOID_AIR,
            Blocks.WATER, Blocks.LAVA,
            Blocks.DIRT, Blocks.GRASS_BLOCK, Blocks.COARSE_DIRT, Blocks.ROOTED_DIRT, Blocks.PODZOL, Blocks.MYCELIUM,
            Blocks.STONE, Blocks.DEEPSLATE, Blocks.GRANITE, Blocks.DIORITE, Blocks.ANDESITE,
            Blocks.TUFF, Blocks.CALCITE,
            Blocks.SAND, Blocks.RED_SAND, Blocks.GRAVEL,
            Blocks.SNOW, Blocks.SNOW_BLOCK, Blocks.ICE, Blocks.PACKED_ICE, Blocks.BLUE_ICE,
            Blocks.CLAY, Blocks.MUD, Blocks.SOUL_SAND, Blocks.SOUL_SOIL,
    ...
    /**
     * Attempts to find a real structure block near the approximate position.
     *
     * @param level       the server level
     * @param approxPos   approximate position from the structure locate call
        ...
        try {
            ...
            for (StructurePiece piece : pieces) {
                ...
                for (int y = bb.maxY(); y >= bb.minY() && blocksScanned < MAX_BLOCKS_TO_SCAN; y--) {
                    for (int x = bb.minX(); x <= bb.maxX() && blocksScanned < MAX_BLOCKS_TO_SCAN; x++) {
                        for (int z = bb.minZ(); z <= bb.maxZ() && blocksScanned < MAX_BLOCKS_TO_SCAN; z++) {
                            blocksScanned++;
            ...
            // Fallback: return center of the first piece's bounding box
