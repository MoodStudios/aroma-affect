...
public final class NoseInventoryUi {
    ...
    private static void positionButton(AbstractContainerScreen<?> screen, NoseStrapToggleButton button) {
        ...
        if (!(screen instanceof AbstractContainerScreenAccessor accessor) || headSlotOpt.isEmpty()) {
    ...
    private static Optional<Slot> findHeadSlot(AbstractContainerMenu menu) {
        for (Slot slot : menu.slots) {
            if (slot.container instanceof Inventory && slot.getContainerSlot() == HEAD_CONTAINER_SLOT) {
