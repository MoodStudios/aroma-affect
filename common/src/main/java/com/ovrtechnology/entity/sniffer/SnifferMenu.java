...
public class SnifferMenu extends AbstractContainerMenu {
    ...
    @Getter private final Sniffer sniffer;

    @SuppressWarnings("this-escape")
    public SnifferMenu(
            int containerId,
            Inventory playerInventory,
            SnifferContainer snifferContainer,
            Sniffer sniffer) {
        ...
        this.addSlot(
                new Slot(snifferContainer, SnifferContainer.SADDLE_SLOT, 8, 18) {
                    @Override
        ...
        this.addSlot(
                new Slot(snifferContainer, SnifferContainer.DECORATION_SLOT, 8, 36) {
                    @Override
                    public boolean mayPlace(ItemStack stack) {
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 9; col++) {
                this.addSlot(
                        new Slot(playerInventory, 9 + row * 9 + col, 8 + col * 18, 84 + row * 18));
            }
        }
            ...
            if (index < 2) {
                ...
                if (itemStack.is(Items.SADDLE)) {
                    ...
                    if (!this.moveItemStackTo(
                            itemStack, SnifferContainer.DECORATION_SLOT, 2, false)) {
                        return ItemStack.EMPTY;
                    }
                } else {
