...
public final class ResourceManagerDataSource implements DataSource {
    ...
    public JsonElement read(String classpathPath) {
        Identifier loc = toResourceLocation(classpathPath);
        if (loc == null) {
            return null;
        }
        ...
        try (BufferedReader reader = resource.get().openAsReader()) {
            ...
            AromaAffect.LOGGER.error(
                    "Error reading resource {} ({}): {}", loc, classpathPath, e.getMessage());
            return null;
        }
    }
    ...
    public Map<Identifier, JsonElement> listJson(String directory) {
        ...
        for (Map.Entry<Identifier, Resource> entry : found.entrySet()) {
            ...
            try (BufferedReader reader = entry.getValue().openAsReader()) {
                ...
                Identifier keyLoc =
                        Identifier.fromNamespaceAndPath(loc.getNamespace(), name);
    ...
    @Nullable
    private static Identifier toResourceLocation(String classpathPath) {
        if (classpathPath == null || classpathPath.isEmpty()) {
            return null;
        }
        String stripped =
                classpathPath.startsWith("data/")
