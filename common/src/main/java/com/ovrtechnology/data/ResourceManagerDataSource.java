...
public final class ResourceManagerDataSource implements DataSource {
    ...
    public JsonElement read(String classpathPath) {
        Identifier loc = toIdentifier(classpathPath);
        if (loc == null) {
            return null;
        }
    ...
    public Map<Identifier, JsonElement> listJson(String directory) {
        ...
        Map<Identifier, Resource> found = resourceManager.listResources(directory,
        ...
        for (Map.Entry<Identifier, Resource> entry : found.entrySet()) {
            ...
            try (BufferedReader reader = entry.getValue().openAsReader()) {
                ...
                Identifier keyLoc = Identifier.fromNamespaceAndPath(loc.getNamespace(), name);
