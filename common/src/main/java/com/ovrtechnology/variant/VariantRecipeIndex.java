...
public final class VariantRecipeIndex {
    ...
    private static final Map<Identifier, Entry> BY_VARIANT = new HashMap<>();

    private VariantRecipeIndex() {}

    public static Optional<Entry> get(Identifier variantId) {
        return Optional.ofNullable(BY_VARIANT.get(variantId));
    }

    public static void reload(DataSource source) {
        BY_VARIANT.clear();
        Map<Identifier, JsonElement> recipes = source.listJson("recipe");
        for (Map.Entry<Identifier, JsonElement> entry : recipes.entrySet()) {
            try {
                ...
                Identifier variantId = Ids.parse(variantStr);
