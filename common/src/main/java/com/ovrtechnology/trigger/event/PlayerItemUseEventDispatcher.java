...
public final class PlayerItemUseEventDispatcher {
    ...
    private static void dispatchPotion(ItemStack stack, EventTriggersConfig config) {
        ...
        Identifier primaryEffectId = null;
        for (MobEffectInstance effect : potionHolder.value().getEffects()) {
            Identifier rl = mobEffectId(effect.getEffect());
            if (rl != null) {
                primaryEffectId = rl;
                break;
            }
        ...
        Identifier potionId = potionHolder.unwrapKey().map(k -> k.identifier()).orElse(null);
    ...
    private static Identifier mobEffectId(Holder<MobEffect> effectHolder) {
        if (effectHolder == null) return null;
        return effectHolder.unwrapKey().map(k -> k.identifier()).orElse(null);
    }
